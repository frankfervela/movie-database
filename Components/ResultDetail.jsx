    ...
    return (
        <div className="flex bg-gray-800 p-5 rounded-xl flex-col md:flex-row flex-grow mx-5 my-10 ">
            <Image className={"object-cover rounded-xl"} height={1080} width={1920}
                   src={`${BASE_URL}${movieResult.backdrop_path || movieResult.poster_path}`}/>
            <div className={"flex flex-col m-6"}>
                <h3 className="font-bold mb-3 text-xl md:text-3xl lg:text-5xl text-gray-200">{movieResult.title || movieResult.name} ({movieResult.release_date?.substr(0,4) || movieResult.first_air_date?.substr(0,4)})</h3>
                <div className={"flex flex-row m-1"}>
                    <p className={"inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none text-red-100 bg-blue-500 rounded-full"}>Length: {movieResult.runtime || movieResult.number_of_episodes}m</p>
