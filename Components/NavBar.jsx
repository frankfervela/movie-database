        ...
        <nav className={"relative"}>
            <div className={" last:pr-10 flex px-10 sm:px-20 text-2xl whitespace-nowrap space-x-10 sm:space-x-20 overflow-x-scroll scrollbar-hide"}>
                ...
                {Object.entries(requests).map(([key, {title, url}]) => (
                    <h2 key={key} className={"cursor-pointer transition duration-100 transform hover:scale-125 hover:text-white active:text-red-400"} onClick={() => router.push(`/?genre=${key}`)}>{title}</h2>
                ))}
